...
export default function QuintaFeira({ navigation }) {
  const { diaSemana } = useTreinos();
  const { removeTreinoDia } = useTreinos();
  const treinos = diaSemana.quinta || [];
  ...
  const handleDelete = () => {
    if (selectedTreino) {
      removeTreinoDia('quinta', selectedTreino.key);
  ...
  const renderLista = ({ item }) => {
    ...
    return (
      <Grid>
        <Text style={styles.containerText}>Treinos de Quinta-Feira</Text>
        ...
        <>
          <Grid style={styles.buttonContainer}>
            <Button
              ...
              style={styles.button}
              onPress={() =>
                navigation.navigate('SelecionarTreino', { dia: 'quinta' })
              }
            >
              Adicionar treino
            </Button>
