...
export default function QuartaFeira({ navigation }) {
  const { diaSemana } = useTreinos();
  const { removeTreinoDia } = useTreinos();
  const treinos = diaSemana.quarta || [];
  ...
  const handleDelete = () => {
    if (selectedTreino) {
      removeTreinoDia('quarta', selectedTreino.key);
  ...
  const renderLista = ({ item }) => {
    ...
    return (
      <Grid>
        <Text style={styles.containerText}>Treinos de Quarta-Feira</Text>
        ...
        <>
          <Grid style={styles.buttonContainer}>
            <Button
              ...
              style={styles.button}
              onPress={() =>
                navigation.navigate('SelecionarTreino', { dia: 'quarta' })
              }
            >
              Adicionar treino
            </Button>
