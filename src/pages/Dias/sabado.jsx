...
export default function Sabado({ navigation }) {
  const { diaSemana } = useTreinos();
  const { removeTreinoDia } = useTreinos();
  const treinos = diaSemana.sabado || [];
  ...
  const handleDelete = () => {
    if (selectedTreino) {
      removeTreinoDia('sabado', selectedTreino.key);
  ...
  const renderLista = ({ item }) => {
    ...
    return (
      <Grid>
        <Text style={styles.containerText}>Treinos de Sabado</Text>
        ...
        <>
          <Grid style={styles.buttonContainer}>
            <Button
              ...
              style={styles.button}
              onPress={() =>
                navigation.navigate('SelecionarTreino', { dia: 'sabado' })
              }
            >
              Adicionar treino
            </Button>
