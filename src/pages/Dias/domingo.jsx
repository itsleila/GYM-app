...
export default function Domingo({ navigation }) {
  const { diaSemana } = useTreinos();
  const { removeTreinoDia } = useTreinos();
  const treinos = diaSemana.domingo || [];
  ...
  const handleDelete = () => {
    if (selectedTreino) {
      removeTreinoDia('domingo', selectedTreino.key);
  ...
  const renderLista = ({ item }) => {
    ...
    return (
      <Grid>
        <Text style={styles.containerText}>Treinos de Domingo</Text>
        ...
        <>
          <Grid style={styles.buttonContainer}>
            <Button
              ...
              style={styles.button}
              onPress={() =>
                navigation.navigate('SelecionarTreino', { dia: 'domingo' })
              }
            >
              Adicionar treino
            </Button>
