...
export default function SextaFeira({ navigation }) {
  const { diaSemana } = useTreinos();
  const { removeTreinoDia } = useTreinos();
  const treinos = diaSemana.sexta || [];
  ...
  const handleDelete = () => {
    if (selectedTreino) {
      removeTreinoDia('sexta', selectedTreino.key);
  ...
  const renderLista = ({ item }) => {
    ...
    return (
      <Grid>
        <Text style={styles.containerText}>Treinos de Sexta-Feira</Text>
        ...
        <>
          <Grid style={styles.buttonContainer}>
            <Button
              ...
              style={styles.button}
              onPress={() =>
                navigation.navigate('SelecionarTreino', { dia: 'sexta' })
              }
            >
              Adicionar treino
            </Button>
