...
export default function TercaFeira({ navigation }) {
  const { diaSemana } = useTreinos();
  const { removeTreinoDia } = useTreinos();
  const treinos = diaSemana.terca || [];
  ...
  const handleDelete = () => {
    if (selectedTreino) {
      removeTreinoDia('terca', selectedTreino.key);
  ...
  const renderLista = ({ item }) => {
    ...
    return (
      <Grid>
        <Text style={styles.containerText}>Treinos de Terça-Feira</Text>
        ...
        <>
          <Grid style={styles.buttonContainer}>
            <Button
              ...
              style={styles.button}
              onPress={() =>
                navigation.navigate('SelecionarTreino', { dia: 'terça' })
              }
            >
              Adicionar treino
            </Button>
